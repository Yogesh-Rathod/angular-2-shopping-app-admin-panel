...
import { ProductsService, XlsxToJsonService, JsonToExcelService, CatalogManagementService } from 'app/services';
...
export class CatalogBulkUploadComponent implements OnInit {
    ...
    sendApproval(event) {
        this.showLoader = true;
        console.log("this.productsInfo ", this.productsInfo);
            ...
            this.catalogManagementService
                ...
                .then((success) => {
                    if (success.Code === 200 && success.Data && success.Data.length === 0) {
                        ...
                        this.toastr.error('Oops! Could not send products for approval.', 'Error!');
                    }
                    this.showLoader = false;
                }).catch((error) => {
                    this.showLoader = false;
