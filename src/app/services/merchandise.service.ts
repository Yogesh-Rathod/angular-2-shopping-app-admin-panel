...
export class MerchandiseService {
    ...
    getCategories(categoryId?) {
        let url = categoryId
            ? `${environment.merchandiseUrl}Merchandise/Category/${categoryId}`
            : `${environment.merchandiseUrl}Merchandise/Category`;
        this.headers.set(
            'Authorization',
    ...
    getCategoriesByLevel(level) {
        ...
        }Merchandise/CategoriesByLevel/${level}`;
        this.headers.set(
            'Authorization',
            this.commonAppSer.crateAuthorization()
        );
    ...
    getUnApprovedCategories() {
        let url = `${
            environment.merchandiseUrl
        }Merchandise/CategoriesForApproval`;
        this.headers.set(
            'Authorization',
            this.commonAppSer.crateAuthorization()
        );
    ...
    addCategory(categoryInfo) {
        const url = `${
            environment.merchandiseUrl
        }Merchandise/AddToApprovalCategory`;
        this.headers.set(
            'Authorization',
            this.commonAppSer.crateAuthorization()
        );
        this.headers.set(
            'LRSignAuth',
            this.commonAppSer.createHMACSignature('POST', url, categoryInfo)
        );
        return this.http
            .post(url, JSON.stringify(categoryInfo), this.options)
            .toPromise()
            .then(this.responseHandler.handleResponse)
            .catch(err => this.responseHandler.handleError(err));
    }
