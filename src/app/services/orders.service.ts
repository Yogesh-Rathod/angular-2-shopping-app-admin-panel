...
export class OrdersService {
    ...
    getOrdersByPONumber(poNumber?, queryParams?, pageSize: any = 150) {
        let url = poNumber
            ? `${
              ...
              }Orders/${poNumber}?e.pageSize=${pageSize}`
            : `${environment.merchandiseUrl}Orders?e.pageSize=${pageSize}`;
        this.headers.set(
            'Authorization',
    ...
    sendToProcessed(orders) {
        ...
        this.headers.set(
            'LRSignAuth',
            this.commonAppSer.createHMACSignature('PUT', url, orders)
        );
        return this.http
            .put(url, JSON.stringify(orders), this.options)
            .toPromise()
            .then(this.responseHandler.handleResponse)
            .catch(err => this.responseHandler.handleError(err));
    }
