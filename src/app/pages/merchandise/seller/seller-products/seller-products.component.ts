...
export class SellerProductsComponent implements OnInit {
    ...
    getAllProducts() {
        ...
        this.productsService
            .getProducts(null, 1, this.showRecords)
            .then(products => {
                this.products = products.Data ? products.Data.Products : [];
                this.totalRecords = products.Data
    ...
    exportAllProducts(searchProductForm) {
        ...
        if (this.atLeastOneFieldRequires(searchProductForm, true)) {
            ...
            this.productsService
                .getProductsForExport(searchProductForm, 1, this.totalRecords)
                .then(products => {
            ...
            this.productsService
                .getProductsForExport(null, 1, this.totalRecords)
                .then(products => {
                    products = products.Data ? products.Data.Products : [];
                    if (products.length > 0) {
                        this.jsonToExcelService.exportAsExcelFile(
    ...
    searchProduct(searchProductForm) {
        ...
        if (!this.atLeastOnePresent) {
            ...
            this.productsService
                .getProducts(searchProductForm, 1, this.showRecords)
                .then(products => {
    ...
    pageChanged($event) {
        ...
        this.productsService
            .getProducts(searchProductForm, this.p, this.showRecords)
            .then(products => {
                this.products = products.Data.Products;
                this.totalRecords = products.Data.TotalRecords;
                this.bigLoader = false;
        ...
        this.recordsToSkip =
            this.p > 1 ? (this.p - 1) * this.showRecords + 1 : 1;
    ...
    exportProducts() {
        ...
        if (this.selectAllCheckbox) {
            ...
            _.forEach(products, item => {
                ...
                delete item.RetailPriceInclusive;
        ...
        this.jsonToExcelService.exportAsExcelFile(products, 'seller_products');
    }

    bulkUpload() {
        const activeModal = this.modalService.open(SellsBulkUploadComponent, {
            size: 'sm'
        });
        activeModal.result
            .then(status => {
    ...
    sendForApproval() {
        ...
        _.forEach(this.products, item => {
            if (item.isChecked) {
                if (item.Status === 'Draft') {
                    ...
                    this.errorMessage.status = true;
                    this.errorMessage.message =
                        'In order to send for approval product status should be draft.';
                    this.approveLoader = false;
                    $('[data-toggle="tooltip"]').tooltip('hide');
                    return;
                }
        ...
        if (!this.errorMessage.status) {
            $('[data-toggle="tooltip"]').tooltip('hide');
            this.productsService
                .sendproductForApproval(productsToConfirm, searchProductForm)
                .then(res => {
                    if (res.Code === 200) {
                        this.getAllProducts();
                        this.toastr.success(
